...
void PctMapCleaner::validate() const {
  ...
  if (config_.sor_search_backend != SorSearchBackend::Legacy &&
      config_.sor_search_backend != SorSearchBackend::SingleExact) {
    throw std::invalid_argument("invalid SOR search backend");
  }
...
PointCloudXYZ PctMapCleaner::statistical_filter(
  ...
  flann::IndexParams index_params = flann::KDTreeIndexParams(4);
  if (config_.sor_search_backend == SorSearchBackend::SingleExact) {
    index_params = flann::KDTreeSingleIndexParams(10, true);
  }
  flann::Index<flann::L2<float>> index(dataset, index_params);
  index.buildIndex();
  timer.mark("sor.build");
  std::vector<double> mean_distances(count, 0.0);
  flann::SearchParams search_params(64, 0.0F, true);
  search_params.cores = worker_count(config_.threads);
  constexpr std::size_t block_size = 4096U;
  const std::size_t capacity = std::min(block_size, count);
  const auto invalid_index = std::numeric_limits<std::size_t>::max();
  std::vector<std::size_t> neighbor_indices(capacity * query_count);
  std::vector<float> neighbor_distances(capacity * query_count);
  for (std::size_t begin = 0; begin < count; begin += block_size) {
    const std::size_t rows = std::min(block_size, count - begin);
    std::fill_n(neighbor_indices.begin(), rows * query_count, invalid_index);
    flann::Matrix<float> queries(&data[begin * 3U], rows, 3);
    flann::Matrix<std::size_t> result_indices(neighbor_indices.data(), rows, query_count);
    flann::Matrix<float> result_distances(neighbor_distances.data(), rows, query_count);
    // knnSearch returns a total for all rows, not a per-query neighbor count.
    index.knnSearch(queries, result_indices, result_distances, query_count, search_params);
    for (std::size_t row = 0; row < rows; ++row) {
      const std::size_t point_index = begin + row;
      double total = 0.0;
      int used = 0;
      for (std::size_t neighbor = 0; neighbor < query_count; ++neighbor) {
        const std::size_t offset = row * query_count + neighbor;
        const auto neighbor_index = neighbor_indices[offset];
        if (neighbor_index == invalid_index || neighbor_index == point_index) continue;
        total += std::sqrt(std::max(0.0F, neighbor_distances[offset]));
        ++used;
