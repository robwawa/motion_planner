...
namespace unitree_legged_control 
{
    ...
    UnitreeJointController::UnitreeJointController(){
        memset(&lastCmd, 0, sizeof(pct_scan_gazebo::MotorCmd));
        memset(&lastState, 0, sizeof(pct_scan_gazebo::MotorState));
    ...
    void UnitreeJointController::setCommandCB(const pct_scan_gazebo::MotorCmdConstPtr& msg)
    {
    ...
    bool UnitreeJointController::init(hardware_interface::EffortJointInterface *robot, ros::NodeHandle &n)
    {
        ...
        // pub_state = n.advertise<pct_scan_gazebo::MotorState>(name_space + "/state", 20); 
        // Start realtime state publisher
        controller_state_publisher_.reset(
            new realtime_tools::RealtimePublisher<pct_scan_gazebo::MotorState>(n, name_space + "/state", 1));        
